...
private[kafka010] object KafkaWriter extends Logging {
  ...
  def write(
    ...
    queryExecution.toRdd.mapPartitionsWithIndexInternal({ (idx, iter) =>
      val writeTask = new KafkaWriteTask(kafkaParameters, schema, idx, topic)
      Utils.tryWithSafeFinally(block = writeTask.execute(iter))(
        finallyBlock = writeTask.close())
      iter
    }, preservesPartitioning = true).foreachPartition(_ => ())
  }
}
